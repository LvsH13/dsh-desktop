...
var MESSAGES = {
  ...
  setAutoStartMode: '已更新开机打开方式（下次登录生效）',
...
var AUTO_START_MODES = [
  { value: 'desktop', label: '桌面端（独立窗口）' },
  { value: 'web', label: '网页端（默认浏览器）' },
];
...
/**
 * Shell rendered immediately, before any snapshot arrives.
 *
 * The panel used to render nothing but "加载中…" until the whole server-side
 * aggregate resolved — and that aggregate could cold-start a tasklist /
 * PowerShell probe, so the user stared at an empty box for a second or more.
 * The static frame is free, so render it at once and reserve placeholders for
 * the values that actually need the snapshot.
 */
  ...
  return React.createElement('div', { className: 'dshd-settings' },
    ...
    React.createElement('section', { className: 'dshd-panel' },
      React.createElement('div', { className: 'dshd-panel-title' },
        ...
        React.createElement('p', null, '正在读取桌面端状态…')),
      React.createElement('div', { className: 'dshd-grid' },
        ['Harness 服务', '系统托盘', '桌面窗口', '桌面快捷方式', '开机自启动', '终端界面'].map(function (label) {
          return React.createElement('div', { className: 'dshd-card', key: label, 'data-status': 'pending' },
            React.createElement('span', null, label),
            React.createElement('strong', { className: 'dshd-skeleton' }, '读取中…'));
        }))));
}
...
function DesktopView({ controller }) {
  ...
  React.useEffect(() => {
    ...
    // 15 秒而不是 5 秒: 每次刷新都是服务端一次聚合快照 (含进程探测), 5 秒的
    // 节奏在面板打开期间会持续产生后台探测开销, 对"反映真实模式"没有必要。
  ...
  if (state.status === 'idle' || (state.status === 'loading' && snapshot === undefined)) {
    return React.createElement(SettingsSkeleton, null);
  }
  ...
  return React.createElement('div', { className: 'dshd-settings' },
    ...
    React.createElement('section', { className: 'dshd-panel' },
      React.createElement('div', { className: 'dshd-panel-title' },
        ...
        React.createElement('p', null, '登录 Windows 后自动拉起 Harness，并按下面的选择只打开其中一端。')),
      React.createElement('label', { className: 'dshd-row' },
        React.createElement('span', { className: 'dshd-row-label' },
          React.createElement('strong', null, '开机自动启动'),
      ...
      React.createElement('label', { className: 'dshd-row' },
        React.createElement('span', { className: 'dshd-row-label' },
          React.createElement('strong', null, '开机打开方式'),
          React.createElement('small', null, '桌面端 = 打开独立应用窗口；网页端 = 用默认浏览器打开 Harness 界面。只打开所选的那一端，不会同时开两个窗口。修改后下次登录生效。')),
        React.createElement('select', {
          className: 'dshd-select',
          value: value.autoStartMode === 'web' ? 'web' : 'desktop',
          disabled: busy || !s.writable || !value.autoStart,
          onChange: (e) => { void controller.act('setAutoStartMode', { mode: e.target.value, expectedRevision: s.settings.revision }); },
        }, AUTO_START_MODES.map(function (mode) {
          return React.createElement('option', { key: mode.value, value: mode.value }, mode.label);
        })))),
    React.createElement('section', { className: 'dshd-panel' },
      React.createElement('div', { className: 'dshd-panel-title' },
...
.dshd-card[data-status=pending]{border-left-color:#c8c4bc}
...
.dshd-skeleton{color:var(--dsw-alias-fg-muted,#77736d);font-weight:400}
.dshd-select{height:30px;padding:0 8px;border-radius:8px;border:1px solid var(--dsw-alias-border-subtle,#dedbd5);background:var(--dsw-alias-bg-layer-1,#fff);color:inherit;font-size:12px;font-weight:600;cursor:pointer;flex:none;min-width:168px}
.dshd-select:disabled{opacity:.5;cursor:default}
