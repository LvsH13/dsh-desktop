  ...
  return React.createElement('div', { className: 'dshd-settings' },
    React.createElement('header', { className: 'dshd-header' },
      React.createElement('div', null,
        ...
        React.createElement('p', null, 'Windows 桌面伴侣：系统托盘黑鲸图标、桌面快捷方式与开机自启动。默认以独立桌面窗口打开（本机 Chromium 浏览器应用模式——优先默认浏览器，否则 Edge/Chrome——+ 独立配置目录，按主屏比例 1352×972 居中，可缩放/最大化/最小化，窗口使用黑鲸图标），可与网页端一键互切（切换时另一端的窗口会自动关闭）。托盘右键可随时退出。')),
    ...
    React.createElement('section', { className: 'dshd-panel' },
      React.createElement('div', { className: 'dshd-panel-title' },
        ...
        React.createElement('p', null, 'Harness 服务与桌面端各部分的当前状态。')),
    ...
    React.createElement('section', { className: 'dshd-panel' },
      React.createElement('div', { className: 'dshd-panel-title' },
        ...
        React.createElement('p', null, '登录 Windows 后自动在系统托盘显示 DeepSeek Harness 图标。')),
      React.createElement('label', { className: 'dshd-row' },
        React.createElement('span', { className: 'dshd-row-label' },
          ...
          React.createElement('small', null, '写入 HKCU Run 注册表项，无需管理员权限')),
    ...
    React.createElement('section', { className: 'dshd-panel' },
      React.createElement('div', { className: 'dshd-panel-title' },
        ...
        React.createElement('p', null, '默认以独立桌面窗口打开（本机 Chromium 浏览器应用模式——优先默认浏览器，否则 Edge/Chrome——+ 独立配置目录，不受浏览器运行状态影响）：按主屏比例 1352×972 计算尺寸（1920×1080 主屏下即 1352×972）并居中于显示器，最小 960×690，可自由缩放、最大化、最小化与移动，渲染就绪前先隐藏避免闪烁，并适配 125%/150% 高 DPI 缩放；窗口使用快捷方式中的黑鲸图标。桌面端与网页端可一键互切，切换时另一端的窗口会自动关闭。终端窗口默认隐藏，只有在这里开启后才会在启动时显示。')),
...
.dshd-header{display:flex;justify-content:space-between;gap:20px;align-items:flex-start;padding:8px 2px}
.dshd-header h2{font-size:25px;letter-spacing:-.025em;margin:3px 0 6px}
...
.dshd-release{display:grid;gap:4px;min-width:200px;padding:9px 11px;border-radius:10px;background:var(--dsw-alias-bg-layer-2,#f7f5f1);font-size:10px;color:var(--dsw-alias-fg-muted,#77736d)}
.dshd-release span{display:flex;justify-content:space-between;gap:12px}
.dshd-release strong{color:var(--dsw-alias-fg-primary,#26231f);font-weight:650}
.dshd-lastopen{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px 12px;border-radius:10px;background:var(--dsw-alias-bg-layer-2,#f7f5f1);font-size:12px}
.dshd-lastopen span{color:var(--dsw-alias-fg-muted,#77736d)}
.dshd-lastopen strong{font-weight:650}
...
.dshd-panel-title{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}
.dshd-panel-title h3{font-size:14px;margin:0}
.dshd-panel-title p{font-size:11px;line-height:1.45;color:var(--dsw-alias-fg-muted,#77736d);margin:4px 0 0;max-width:620px}
.dshd-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px}
...
.dshd-btn{display:inline-flex;align-items:center;height:30px;padding:0 14px;border-radius:8px;border:1px solid var(--dsw-alias-border-subtle,#dedbd5);background:var(--dsw-alias-bg-layer-1,#fff);color:inherit;font-size:12px;font-weight:600;cursor:pointer}
