...
/**
 * Whether the Edge/Chrome app-mode desktop window for this origin is running.
 *
 * Fast path: read the tray's app-window.json record (self-healed by the
 * watcher to the real window owner) and probe the PID. This avoids
 * cold-starting a PowerShell process on every settings snapshot (1-3s).
 * Falls back to the tray script when the record is missing or unreadable.
 */
let appWindowFast = { at: 0, active: false };
let appWindowSlow = { at: 0, active: false };
const APP_WINDOW_FAST_TTL = 2000;
const APP_WINDOW_SLOW_TTL = 5000;
...
async function appWindowActive() {
  ...
  try {
    ...
    if (isRecord(parsed) && typeof parsed.pid === 'number' && parsed.pid > 0) pid = parsed.pid;
  } catch {
    /* no record yet */
  }
  if (pid > 0) {
    ...
    appWindowFast = { at: now, active: alive };
    return alive;
  }
...
export async function apply(ctx, config = {}) {
  ...
  writeHarnessInfo();
  // Keep the login auto-start registry key in sync with the stored setting.
  await syncAutoStart(settings.get().autoStart);
  // Fix the desktop shortcut when it still points at the legacy script
  // ($DSH_HOME\desktop, v0.1): the old script opened the browser instead of
  // the app-mode desktop window.
  if (!(await syncShortcut())) {
    ctx.logger.warn('dsh-desktop shortcut refresh failed');
  }
  // Replace any stale tray (v0.1 copy from the legacy directory may still run
  // the old npx-based script), then bring the tray companion up whenever the
  // harness starts, so the whale icon is in the notification area as soon as
  // DSH opens. Idempotent: the tray script is single-instance (named mutex).
  await stopTray();
  spawnHidden([TRAY_SCRIPT]);
