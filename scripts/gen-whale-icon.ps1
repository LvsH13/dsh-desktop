# Generates dsh-whale.ico (multi-size) from the DeepSeek Harness wordmark whale
# SVG path. Renders with WPF PathGeometry at every classic icon size and packs
# the PNGs into a single ICO (Vista+ PNG-compressed entries).
param(
  [string]$OutFile = (Join-Path $PSScriptRoot '..\assets\dsh-whale.ico'),
  [string]$PreviewPng = (Join-Path $PSScriptRoot '..\assets\dsh-whale-256.png')
)
$ErrorActionPreference = 'Stop'
Add-Type -AssemblyName PresentationCore
Add-Type -AssemblyName WindowsBase

$pathData = 'M23.0584 4.95203C22.8129 4.83203 22.7074 5.06103 22.5639 5.17704C22.5149 5.21454 22.4734 5.26354 22.4319 5.30854C22.0734 5.69155 21.6543 5.94306 21.1073 5.91306C20.3073 5.86806 19.6243 6.11957 19.0203 6.73158C18.8918 5.97706 18.4652 5.52655 17.8162 5.23754C17.4767 5.08753 17.1332 4.93703 16.8952 4.61052C16.7292 4.37801 16.6837 4.11901 16.6007 3.8635C16.5477 3.70949 16.4952 3.55199 16.3177 3.52549C16.1252 3.49549 16.0497 3.65699 15.9742 3.792C15.6722 4.34401 15.5552 4.95203 15.5667 5.56805C15.5932 6.95359 16.1782 8.05712 17.3407 8.84215C17.4727 8.93215 17.5067 9.02215 17.4652 9.15366C17.3857 9.42416 17.2917 9.68667 17.2087 9.95718C17.1557 10.1297 17.0767 10.1677 16.8917 10.0922C16.2537 9.82568 15.7027 9.43117 15.2156 8.95465C14.3891 8.15513 13.6416 7.2726 12.7096 6.58158C12.4906 6.42007 12.2716 6.27007 12.045 6.12707C11.094 5.20354 12.1696 4.44502 12.4186 4.35501C12.6791 4.26101 12.5091 3.938 11.6675 3.942C10.826 3.9455 10.056 4.22751 9.07446 4.60302C8.93096 4.65952 8.77995 4.70052 8.62545 4.73452C7.73492 4.56552 6.80989 4.52802 5.84386 4.63702C4.02481 4.83953 2.57177 5.69955 1.50373 7.1676C0.220694 8.93215 -0.0813148 10.9372 0.288196 13.0283C0.676708 15.2323 1.80174 17.0569 3.53029 18.4834C5.32285 19.9625 7.38741 20.6875 9.74298 20.5485C11.1735 20.466 12.7661 20.2745 14.5626 18.7539C15.0156 18.9795 15.4912 19.0695 16.2797 19.137C16.8872 19.1935 17.4722 19.107 17.9252 19.013C18.6347 18.8629 18.5857 18.2059 18.3292 18.0854C16.2497 17.1169 16.7062 17.5109 16.2912 17.1919C17.3477 15.9419 18.9618 13.7198 19.4598 10.6942C19.5088 10.3602 19.5713 9.88968 19.5638 9.61917C19.5598 9.45417 19.5978 9.39016 19.7863 9.37116C20.3073 9.31116 20.8128 9.16866 21.2773 8.91315C22.6249 8.17713 23.1684 6.96809 23.2964 5.51905C23.3154 5.29754 23.2924 5.06853 23.0584 4.95203ZM11.3165 17.9954C9.30097 16.4109 8.32344 15.8894 7.91992 15.9119C7.54241 15.9344 7.61042 16.3664 7.69342 16.6479C7.78042 16.9259 7.89342 17.1174 8.05193 17.3614C8.16143 17.5229 8.23694 17.7629 7.94243 17.9434C7.29341 18.3449 6.16487 17.8084 6.11187 17.7819C4.79833 17.0084 3.7003 15.9874 2.92628 14.5908C2.17875 13.2468 1.74474 11.8047 1.67324 10.2657C1.65424 9.89418 1.76374 9.76267 2.13375 9.69517C2.62077 9.60517 3.12278 9.58617 3.6093 9.65767C5.66636 9.95818 7.41741 10.8777 8.88545 12.3348C9.72348 13.1643 10.3575 14.1558 11.0105 15.1243C11.705 16.1529 12.4521 17.1329 13.4036 17.9364C13.7396 18.2179 14.0076 18.4319 14.2641 18.5899C13.4906 18.6764 12.1996 18.6949 11.3165 17.9964V17.9954ZM12.2826 11.7817C12.2826 11.6167 12.4146 11.4852 12.5806 11.4852C12.6181 11.4852 12.6521 11.4927 12.6826 11.5037C12.7241 11.5187 12.7621 11.5412 12.7921 11.5752C12.8451 11.6277 12.8751 11.7027 12.8751 11.7817C12.8751 11.9467 12.7431 12.0782 12.5771 12.0782C12.4111 12.0782 12.2826 11.9467 12.2826 11.7817ZM15.2831 13.3208C15.0906 13.3998 14.8981 13.4673 14.7131 13.4748C14.4261 13.4898 14.1131 13.3733 13.9431 13.2308C13.6791 13.0093 13.4901 12.8853 13.4111 12.4988C13.3771 12.3338 13.3961 12.0782 13.4261 11.9317C13.4941 11.6162 13.4186 11.4137 13.1961 11.2297C13.0151 11.0797 12.7846 11.0382 12.5316 11.0382C12.4371 11.0382 12.3506 10.9967 12.2861 10.9632C12.1806 10.9107 12.0936 10.7792 12.1766 10.6177C12.2031 10.5652 12.3316 10.4377 12.3616 10.4152C12.7051 10.2197 13.1011 10.2837 13.4676 10.4302C13.8071 10.5692 14.0641 10.8242 14.4336 11.1847C14.8111 11.6202 14.8791 11.7402 15.0941 12.0672C15.2641 12.3228 15.4186 12.5853 15.5247 12.8858C15.5887 13.0733 15.5057 13.2268 15.2831 13.3208Z'

$path = [System.Windows.Media.PathGeometry]::Parse($pathData)
$path = $path.Clone()
$path.FillRule = [System.Windows.Media.FillRule]::Nonzero

# Report winding of every figure (sanity check: inner counters should wind
# opposite to the body for the eye/fin cutouts to appear as holes).
$flat = $path.GetFlattenedPathGeometry()
$i = 0
foreach ($fig in $flat.Figures) {
  $pts = @()
  $pts += $fig.StartPoint
  foreach ($seg in $fig.Segments) {
    if ($seg -is [System.Windows.Media.PolyLineSegment]) { foreach ($p in $seg.Points) { $pts += $p } }
  }
  $area = 0.0
  for ($k = 0; $k -lt $pts.Count - 1; $k++) {
    $area += ($pts[$k].X * $pts[$k + 1].Y) - ($pts[$k + 1].X * $pts[$k].Y)
  }
  $area = $area / 2.0
  Write-Host ("figure {0}: start=({1:N2},{2:N2}) signedArea={3:N3} {4}" -f $i, $fig.StartPoint.X, $fig.StartPoint.Y, $area, $(if ($area -gt 0) { 'CW' } elseif ($area -lt 0) { 'CCW' } else { 'zero' }))
  $i++
}

$bounds = $path.Bounds
Write-Host ("path bounds: x={0:N3} y={1:N3} w={2:N3} h={3:N3}" -f $bounds.X, $bounds.Y, $bounds.Width, $bounds.Height)

$sizes = @(16, 20, 24, 32, 40, 48, 64, 128, 256)
$outDir = Split-Path -Parent $OutFile
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

$pngFiles = @()
foreach ($size in $sizes) {
  $pad = $size * 0.04
  $scale = ($size - 2 * $pad) / [Math]::Max($bounds.Width, $bounds.Height)
  $offsetX = ($size - $bounds.Width * $scale) / 2.0 - $bounds.X * $scale
  $offsetY = ($size - $bounds.Height * $scale) / 2.0 - $bounds.Y * $scale
  $mtx = [System.Windows.Media.Matrix]::new($scale, 0, 0, $scale, $offsetX, $offsetY)

  $g = $path.Clone()
  $g.Transform = [System.Windows.Media.MatrixTransform]::new($mtx)

  $visual = New-Object System.Windows.Media.DrawingVisual
  $dc = $visual.RenderOpen()
  $dc.DrawGeometry([System.Windows.Media.Brushes]::Black, $null, $g)
  $dc.Close()

  $rtb = New-Object System.Windows.Media.Imaging.RenderTargetBitmap($size, $size, 96, 96, [System.Windows.Media.PixelFormats]::Pbgra32)
  $rtb.Render($visual)

  $pngPath = Join-Path $env:TEMP ("dsh-whale-{0}.png" -f $size)
  $enc = New-Object System.Windows.Media.Imaging.PngBitmapEncoder
  $enc.Frames.Add([System.Windows.Media.Imaging.BitmapFrame]::Create($rtb))
  $fs = [System.IO.File]::Open($pngPath, 'Create')
  try { $enc.Save($fs) } finally { $fs.Close() }
  $pngFiles += @{ Size = $size; Path = $pngPath }
  Write-Host ("rendered {0}x{0} -> {1}" -f $size, $pngPath)

  if ($size -eq 256) {
    Copy-Item $pngPath $PreviewPng -Force
    Write-Host ("preview: {0}" -f $PreviewPng)
  }
}

# ---- pack ICO (Vista+ PNG entries) ----
$count = $pngFiles.Count
$header = New-Object byte[] 6
$header[0] = 0; $header[1] = 0
$header[2] = 1; $header[3] = 0   # type: icon
$header[4] = [byte]($count -band 0xFF)
$header[5] = [byte](($count -shr 8) -band 0xFF)

$entries = New-Object System.Collections.Generic.List[byte[]]
$offset = 6 + 16 * $count
foreach ($png in $pngFiles) {
  $bytes = [System.IO.File]::ReadAllBytes($png.Path)
  $entry = New-Object byte[] 16
  $size = $png.Size
  $entry[0] = if ($size -ge 256) { 0 } else { [byte]$size }
  $entry[1] = if ($size -ge 256) { 0 } else { [byte]$size }
  $entry[2] = 0  # colors
  $entry[3] = 0  # reserved
  $entry[4] = 1; $entry[5] = 0   # planes
  $entry[6] = 32; $entry[7] = 0  # bpp
  $entry[8] = [byte]($bytes.Length -band 0xFF)
  $entry[9] = [byte](($bytes.Length -shr 8) -band 0xFF)
  $entry[10] = [byte](($bytes.Length -shr 16) -band 0xFF)
  $entry[11] = [byte](($bytes.Length -shr 24) -band 0xFF)
  $entry[12] = [byte]($offset -band 0xFF)
  $entry[13] = [byte](($offset -shr 8) -band 0xFF)
  $entry[14] = [byte](($offset -shr 16) -band 0xFF)
  $entry[15] = [byte](($offset -shr 24) -band 0xFF)
  $entries.Add($entry)
  $offset += $bytes.Length
}

$out = New-Object System.IO.MemoryStream
$out.Write($header, 0, $header.Length)
foreach ($entry in $entries) { $out.Write($entry, 0, $entry.Length) }
foreach ($png in $pngFiles) {
  $bytes = [System.IO.File]::ReadAllBytes($png.Path)
  $out.Write($bytes, 0, $bytes.Length)
}
[System.IO.File]::WriteAllBytes($OutFile, $out.ToArray())
$out.Dispose()

Write-Host ("ICO written: {0} ({1} bytes, {2} entries)" -f $OutFile, (Get-Item $OutFile).Length, $count)
